...
function getRandomWord(mode) {
	...
	if(mode == "easy") {
		var wordsList = easyWords;
	} else {
		var wordsList = hardWords;
	}
...
function play() {
	...
	var mode = $("#play-btn").attr("class").replace("btn", "").replace("ingame", "").replace(/ /g, "");
	currRecord = 0;
	totalRecord = 0;
...
function getNextFont(mode, currMessage) {
	...
	if(fontsRemaining == 0) {
		...
		if (mode == "hard") {
			...
			for(var i in fontsRemainingArr.fonts) {
				if(fontsRemainingArr.fonts[i].category == randFont.category) {
					otherFontsArr.fonts.push(fontsRemainingArr.fonts[i]);
				}
			}
		} else {
		...
		var fontChoices = [currFont, randFont1.fontName, randFont2.fontName, randFont3.fontName];
		...
		var output = "<table id='game-card'>\n" +
			...
			"</td>\n";
	    output += "<td>\n" +
	    	"<input type='radio' name='selection' value='" + fontChoices[0] + "'>" + fontChoices[0] + "</input>\n" +
	    	"</td>\n";
	    output += "<td>\n" +
	    	"<input type='radio' name='selection' value='" + fontChoices[1] + "'>" + fontChoices[1] + "</input>\n" +
	    	"</td>\n" +
	    	"</tr>\n";
	    output += "<tr>\n" +
	    	"<td>\n" +
	    	"<input type='radio' name='selection' value='" + fontChoices[2] + "'>" + fontChoices[2] + "</input>\n" +
	    	"</td>\n";
	    output += "<td>\n" +
	    	"<input type='radio' name='selection' value='" + fontChoices[3] + "'>" + fontChoices[3] + "</input>\n" +
	    	"</td>\n" +
	    	"</tr>\n";
	    // submit button
	    output += "<tr>\n" +
	    	"<td id='notification-cell' colspan='2'>\n" +
	    	"<h3>" + currMessage + "</h3>" +
	    	"</td>\n" +
	    	"<td id='submit-cell'>\n" +
	    ...
	    document.getElementById("submit-btn").addEventListener('click', function() {checkChoiceHandler(currFont, mode)}, false);
	}

}

function checkChoiceHandler(correctFont, mode) {

	var correctFont = correctFont;
	var mode = mode;
	...
	if($("input[name='selection']").is(":checked")) { // if radio box is checked
		...
		if(selectedFont == correctFont) { // if correct radio box is checked
			...
			getNextFont(mode, correctMessage);
			...
			getNextFont(mode, incorrectMessage);
		}
